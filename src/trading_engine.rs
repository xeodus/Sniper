...
use crate::{
    data::{BotState, Exchange, GridOrder, OrderReq, OrderStatus, Side, Trend, Candles},
    exchange::{binance_auth::Binance, kucoin_auth::KuCoin, RestClient},
    store::OrderStore,
    strategy::{grid_strategy::GridStrategy, trade_strategy::StrategyCalculations},
    websocket::{ws_client::WebSocketClient, binance_ws::BinanceWebSocket, kucoin_ws::KuCoinWebSocket},
...
impl TradingEngine {
    pub async fn new(config: AppConfig, exchange: Exchange) -> TradingResult<Self> {
        let exchange_config = config.get_exchange_config(&format!("{:?}", exchange))?;
        ...
        let ws_config = crate::websocket::ws_client::WebSocketConfig {
            symbol: config.trading.symbol.clone(),
            interval: config.trading.timeframe.clone(),
            max_candles: 1000,
        };

        let ws_client: Box<dyn WebSocketClient + Send + Sync> = match exchange {
            Exchange::Binance => Box::new(BinanceWebSocket::new(ws_config)),
            Exchange::KuCoin => Box::new(KuCoinWebSocket::new(ws_config)),
        };
    ...
    async fn start_websocket(&self) -> TradingResult<()> {
        ...
        tokio::spawn(async move {
            ...
            while let Some(order_req) = rx.recv().await {
                ...
                let result = match exchange_type {
                    Exchange::Binance => {
                        let client = Binance::new(exchange_config.clone());
                        client.place_order(&order_req).await
                    }
                    Exchange::KuCoin => {
                        let client = KuCoin::new(exchange_config.clone());
                        client.place_order(&order_req).await
                    }
                };
                ...
                match result {
                    Ok(response) => {
                        ...
                        if let Ok(mut store) = order_store.write().await {
                        ...
                        // Update state
                        if let Ok(mut state_guard) = state.write().await {
    ...
    async fn start_order_update_processor(&self) {
        ...
        tokio::spawn(async move {
            ...
            while let Some(updated_order) = rx.recv().await {
                ...
                // Update database
                if let Ok(mut store) = update_store.write().await {
                    if let Err(e) = store.db_update_status(&updated_order) {
                ...
                if updated_order.status == OrderStatus::Filled {
                    if let Ok(mut strategy) = grid_strategy.write().await {
                        if let Some(new_order) = strategy.update_grid_on_fill(&updated_order) {
    ...
    async fn start_trading_loop(&self) -> TradingResult<()> {
        ...
        loop {
            ...
            if matches!(current_state.trend, Trend::SideChop) && current_state.open_orders.is_empty() {
                let candles = self.ws_client.read().await.get_candles().await;
                if !candles.is_empty() {
    ...
    pub async fn update_trend(&self, trend: Trend) -> TradingResult<()> {
