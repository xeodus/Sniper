...
impl TradingBot {
    ...
    pub async fn execute_entry_order(&self, signal: Signal, position_side: PositionSide, order_type: OrderType) -> Result<()> {
        let account_balance = *self.account_balace.read().await;

        let (take_profit, stop_loss) = match position_side {
            PositionSide::Long => {
                (signal.price * Decimal::new(104, 2), signal.price * Decimal::new(98, 2))
            },
            PositionSide::Short => {
                (signal.price * Decimal::new(96, 2), signal.price * Decimal::new(102, 2))
            }
        };
