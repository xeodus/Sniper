...
impl BinanceClient {
    ...
    pub async fn place_market_order(&self, req: &OrderReq) -> Result<String> {
        ...
        if req.size.is_zero() {
            return Err(anyhow!("Refusing to place order of size zero for: {}", req.symbol));
        }
        ...
        let response = self.client.post(format!("{}?{}&signature={}", url, body, sign))
            .header("X-MBX-APIKEY", self.api_key.clone()).send().await?;

        if !response.status().is_success() {
            return Err(anyhow!("Invalid response received while placing market order on Binance: {:?}", response.text().await));
        }
    ...
    pub async fn cancel_orders(&self, req: &OrderReq) -> Result<String> {
        ...
        let symbol = req.symbol.replace("/", "").to_uppercase();
        let query_string = format!("symbol={}&originClientOrderId={}&recvWindow=5000&timestamp={}", symbol, req.id, now);
